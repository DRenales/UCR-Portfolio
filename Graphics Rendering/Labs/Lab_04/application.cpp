...
GLuint loadShader(const char *vertexfilename, const char *fragmentfilename) {
  ...
  std::string v = getTextFile(vertexfilename);
  std::string f = getTextFile(fragmentfilename);
  
  const char* v_s = v.c_str();
  const char* f_s = f.c_str();

  program = glCreateProgram();
  vertex_shader_id = glCreateShader(GL_VERTEX_SHADER);
  glShaderSource(vertex_shader_id, 1, &v_s, NULL);
  glCompileShader(vertex_shader_id);
  glAttachShader(program, vertex_shader_id);
  fragment_shader_id = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(fragment_shader_id, 1, &f_s, NULL);
