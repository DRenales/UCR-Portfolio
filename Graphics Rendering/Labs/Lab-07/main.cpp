...
float factorial(int n) { return n == 0 ? 1 : n * factorial(n-1); }
float combination(int n, int k) { factorial(n) / (factorial(k) * factorial(n - k)); }
...
void GL_render()
{
    ...
    for(float t = 0.0; t < 1.0; t += 0.01)
    {
       ...
       {
	        temp += binomial(points.size() - 1, i, t) * points.at(i);
	     }
