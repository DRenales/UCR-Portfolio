...
/*
    ...
    REALLY BIG THANKS TO CODY AND KRISTIAN FOR EVEN REMOTELY GETTING THIS SACK OF GARBAGE COMPLETE!!!!!!
...
//    std::cout<<"CLIPPING"<<endl;
  ...
  vec4 x = in[0]->gl_Position;
  vec4 y = in[1]->gl_Position;
  vec4 z = in[2]->gl_Position;
  ...
  if( x[2] < -x[3] && y[2] < -y[3] && z[2] < -z[2]) return;
  else
  {
	   if( x[2] < -x[3] && y[2] >= -y[3] && z[2] >= -z[3])
     {
	      b = ( -y[2] - y[3] ) / ( x[2] + x[3] - y[2] - y[3] );
	      c = ( -x[2] - x[3] ) / ( z[2] + z[3] - x[2] - x[3] );
	      ...
	      for(int i = 0; i < state.floats_per_vertex; i++)
        {
		        switch(state.interp_rules[i])
            {
                ...
                {
			               a = c * in[2]->gl_Position[3] / (c * in[2]->gl_Position[3] + (1 - c) * in[0]->gl_Position[3]);
			               data1[0].data[i] = a * in[2]->data[i] + (1 - a) * in[0]->data[i];
		            } break;
                ...
                {
			               data1[0].data[i] = c * in[2]->data[i] + (1 - c) * in[0]->data[i];
		            } break;
	     ...
	     for(int i = 0; i < state.floats_per_vertex; ++i)
       {
	        switch(state.interp_rules[i])
          {
		          case interp_type::noperspective:
              {
		              a = b * in[0]->gl_Position[3] / (b * in[0]->gl_Position[3] + (1 - b) * in[1]->gl_Position[3]);
		              data2[0].data[i] = a * in[0]->data[i] + ( 1 - a ) *in [1]->data[i];
		          } break;
              ...
              case interp_type::smooth:
              {
			            data2[0].data[i] = b * in[0]->data[i] + (1 - b) * in[1]->data[i];
		          } break;
